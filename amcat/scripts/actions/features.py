...
class Features(Script):
    ...
    class options_form(forms.Form):
        ...
        unitlevel = forms.ChoiceField(
            [('article', 'article'), ('paragraph', 'paragraph'), ('sentence', 'sentence')], initial='article')
        offset = forms.IntegerField(initial=0)
        batchsize = forms.IntegerField(initial=1000, max_value=2500)
        # usefull for reducing size of output, but perhaps this problem should be for the user and not the server.
        mindocfreq = forms.IntegerField(initial=0)
    ...
    def run(self, _input=None):
        ...
        for a, parnr, sentnr, features in f.streamFeaturesPerUnit(articleset_id=articleset_id, unit_level=unit_level, offset=offset, batchsize=batchsize, verbose=True):
            for feature, count in features.iteritems():
                ...
                if min_docfreq > 1:
                    docfreq[word] += 1
                rows.append({'id': a.id, 'paragraph': parnr, 'sentence':
                            sentnr, 'word': word, 'pos': pos, 'hits': count})
        ...
        if unit_level == 'article':
            columns = ['id', 'word', 'pos', 'hits']
        if unit_level == 'paragraph':
            columns = ['id', 'paragraph', 'word', 'pos', 'hits']
        if unit_level == 'sentence':
            columns = ['id', 'paragraph', 'sentence', 'word', 'pos', 'hits']
