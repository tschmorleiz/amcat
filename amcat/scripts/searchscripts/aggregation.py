...
class TestAggregation(amcattest.AmCATTestCase):
    ...
    def test_dates(self):
        ...
        t = AggregationScript.run_script(
            dict(articlesets=[aset.id], projects=[aset.project_id], dateInterval='month', **base))

        self.assertEqual(set(t.to_list(row_names=True, tuple_name=None)),
                         {('2001-%02i' % i, int(i in (1, 3, 8))) for i in range(1, 9)})

        t = AggregationScript.run_script(
            dict(articlesets=[aset.id], projects=[aset.project_id], dateInterval='quarter', **base))
        self.assertEqual(set(t.to_list(row_names=True, tuple_name=None)),
