...
def run_cli(cls=None, handle_output=None, get_script_depth=2):
    ...
    if cls is None:
        cls = get_script(get_script_depth)
...
def add_argument_from_field(parser, name, field):
    ...
    if field.initial is not None:
        helpfields.append("Default: {field.initial}".format(**locals()))
    ...
    if isinstance(field, forms.ModelMultipleChoiceField) or isinstance(field, forms.MultipleChoiceField):
        args["nargs"] = '+'
        # problem: now the field appears to be optional, but at least it knows which values belong to this argument
        argname = ['--' + optname]
