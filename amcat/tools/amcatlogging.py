...
class ModuleLevelFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if record.exc_info and issubclass(record.exc_info[0], SilentException):
            return False
        # print record, record.name
        if record.levelno < logging.ERROR and record.name in QUIET_MODULES:
            return False
        if record.levelno >= logging.WARNING:
            return True
        if record.levelno >= logging.INFO and record.name in INFO_MODULES:
            return True
        if record.levelno >= logging.DEBUG and record.name in DEBUG_MODULES:
            return True
        return False
