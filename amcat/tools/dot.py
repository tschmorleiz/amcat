...
class Graph(object):
    ...
    def addEdge(self, subj, obj, graph=None, **kargs):
        if type(subj) <> Node:
            subj = self.getNode(subj)
        if type(obj) <> Node:
            obj = self.getNode(obj)
        edge = Edge(subj, obj, **kargs)
        if (subj, obj) not in self.edges:
            self.edges[subj, obj] = []
    ...
    def getDot(self):
        ...
        for i, subgraph in enumerate(self.subgraphs):
            if subgraph:
                entries.append('subgraph cluster_%i {\nlabel="%s"' % (i, subgraph))
            n = set()
    ...
    def normalizeWeights(self, wmin=1, wmax=10):
        ...
        for edges in self.edges.values():
            for edge in edges:
                w = float(edge.weight)
                if max is None or w > max:
                    max = w
                if min is None or w < min:
                    min = w
        if max <> min:
...
class DotTheme(object):
    ...
    def getNodeDot(self, node, graph, subgraph):
        ...
        if hsb:
            attrs['fontcolor'] = "%1.4f,%1.4f,%1.4f" % hsb
