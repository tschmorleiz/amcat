...
def table2unicode(table, colnames=None, formats=None, useunicode=True, box=True, rownames=False, stream=None, encoding=None):
    ...
    def cell(row, col, fmt):
        ...
        if not useunicode and type(val) == unicode:
    ...
    def formatheader(h):
        if not h:
            return h
        if type(h) in (list, tuple, set):
            return ",".join(map(unicode, h))
        return unicode(h)
...
class HTMLGenerator(object):
    ...
    def generateContent(self, table, stream, row):
        ...
        if self.rownames:
            self.element(stream, str(row), "th", **self.getCellAttrs(row=row))
        for col in table.getColumns():
            try:
                val = table.getValue(row, col)
                if val is None:
    ...
    def getCellAttrs(self, row=None, col=None):
        if self.attrhook:
            return self.attrhook(False, row, col)
        return {}

    def getRowAttrs(self, row):
        if self.attrhook:
            return self.attrhook(True, row, None)
        return {}
...
def table2csv(table, colnames=None, csvwriter=None, outfile=sys.stdout, writecolnames=True, writerownames=False,
    ...
    if writecolnames == True:
        writecolnames = lambda col: unicode(col).encode(encoding)
    if writerownames == True:
        writerownames = str
    if writecolnames:
        c = ([""] + cols) if writerownames else cols
        csvwriter.writerow(map(writecolnames, c))
