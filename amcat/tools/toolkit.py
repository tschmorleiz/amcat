...
def _writePrior1900(dt, year, seconds, time):
    ...
    if time:
        date += " %0.2i:%0.2i" % (dt.hour, dt.minute)
...
def executepipe(cmd, listener=None, listenOut=False, outonly=False, **kargs):
    ...
    if outonly:
        e = errr.out.strip()
        if e:
            raise Exception("Error on executing %r:\n%s" % (cmd, e))
        yield outr.out
...
def isSequence(obj, excludeStrings=False):
    ...
    if excludeStrings and isinstance(obj, basestring):
        return False
    if hasattr(obj, "__getslice__"):
        return True
    else:
        return False
...
def HSVtoHTML(h, s, v):
    ...
    if not rgb:
        raise TypeError("Cannot convert hsv (%s,%s,%s)!" % (h, s, v))
