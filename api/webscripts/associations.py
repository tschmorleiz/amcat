...
class AssociationsForm(forms.Form):
    ...
    association_format = forms.ChoiceField(
        label="Number Format", choices=((i, x[0]) for (i, x) in enumerate(FORMATS)), initial=0)
...
class ShowAssociations(WebScript):
    ...
    def run(self):
        ...
        if self.options['network_output'] == 'ool':
            ...
            assocTable = table3.WrappedTable(
                assocTable, cellfunc=lambda a: self.format(a) if isinstance(a, float) else a)
            ...
            if self._interval:
                ...
                columns += [{"mData": cn(colname), "sTitle": colname, "sWidth": "70px", "mDataProp": cn(colname)}
            ...
            for interval in intervals:
                ...
                for i, x, y, a in assocTable:
                    ...
                    opts = {}
                    if self.options['graph_label']:
                        opts['label'] = self.format(a)
                    w = 1 + 10 * a
